...
5. Supabase's built-in email service may be used for project-team recipients. For one designated external acceptance tester, set `ALLOW_MANUAL_TEST_LINKS=true`; an MFA-authenticated administrator can privately hand off the one-time application and first-login links shown after each action. Set it back to `false` and configure Resend custom SMTP and the transactional sending domain before general onboarding.
